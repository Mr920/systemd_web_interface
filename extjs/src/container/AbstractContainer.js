...
Ext.define('Ext.container.AbstractContainer', {
     ...
     * @return {Ext.Element} the focus holding element.
     */
    getFocusEl: function() {
    ...
    doRemove: function(component, doDestroy) {
        ...
        // Only have the layout perform remove postprocessing if the Component is not being destroyed
            ...
            if (hasLayout && !floating && !me.isDestroyed) {
    ...
    moveAfter: function (item, after) {
        ...
        // If undefined, check to see if we're dragging from/to group headers and look up the before item.
        // This is necessary if dragging group headers within a nested stack.
        if (before === undefined && (item.isGroupHeader && after.isGroupHeader)) {
            index = layout.checkNestedHeader(item, after);
            before = items.getAt(index);
        }
