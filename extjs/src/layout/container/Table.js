...
Ext.define('Ext.layout.container.Table', {
    ...
    renderChildren: function() {
        var me = this,
            ...
            trEl, tdEl, itemCt, cellId, oldId, tdId;
        ...
        for (; i < len; i++) {
            ...
            // If we have a cell id, set it if it doesn't match
            if (cellId && cellId != oldId) {
                tdId = cellId;
            // We may have had a cellId before, so we need to clear it out
            // if the component in this slot no longer has one
            } else if (!cellId && !Ext.isAutoId(oldId)) {
                tdId = Ext.id();
            }
            if (tdId) {
                tdEl.dom.id = tdId;
