...
Ext.define('Ext.layout.container.boxOverflow.Menu', {
    ...
    handleOverflow: function(ownerContext) {
        ...
        if (layout.direction !== 'vertical') {
            ...
            if (align.nostretch || align.stretchmax) {
                height = plan.maxSize;
            } else {
                height = plan.targetSize.height;
            }
            posArgs[names.heightIndex] = (height - menuTrigger[names.getHeight]()) / 2;
            menuTrigger.setPosition.apply(menuTrigger, posArgs);
        }
    ...
    showTrigger: function(ownerContext) {
        ...
        if (!ownerContext.boxOptions.align.nostretch) {      
            // Force a height, since the stretch/stretchmax calcs have already happened here
            childContext = ownerContext.context.getCmp(menuTrigger);
            layout.setChildHeight(ownerContext, childContext, menuTrigger.getHeight());
        }
