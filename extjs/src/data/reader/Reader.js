...
Ext.define('Ext.data.reader.Reader', {
     ...
     * Model(s) of the configured Reader. root varies by Reader type.
     * 
     * ##JSON Reader 
     * root is a property name. It may also be a dot-separated 
     * list of property names if the root is nested. The root JSON array will be used
     * by default.
     * 
     * ##XML Reader 
     * root is a CSS selector. The root XML element will be used
     * by default.
     * 
     * ##Array Reader 
     * root is not applicable since the data is assumed to be a
     * single-level array of arrays.
     * 
     * **Note:** root may also be a function that returns the root node from 
     ...
     *     });
     *
     * data.json would return data in the following format:
     *
     *     {
     *         success: true,
     *         items: [{
    ...
    onMetaChange : function(meta) {
        ...
        // Parent proxy is listening to this event to fire its own metachange
        me.fireEventArgs('metachange', [meta]);
    },
    ...
    buildExtractors: function(force) {
        ...
        if (idProp) {
            me.idAccessor = me.createAccessor(idProp);
            
            me.getId = function(record) {
                var id = this.idAccessor(record);
    ...
    recordDataExtractorTemplate : [
        ...
        '%}',
        '<tpl for="fields">',
        '{% var fieldAccessExpression = exprFn.call(reader, values, "__field" + xindex, "source");\n',
        ...
        '    value = {[ exprFn.call(reader, values, "__field" + xindex, "source") ]};\n',
        ...
        '<tpl if="clientIdProp">',
        '    if (record && (internalId = {[ exprFn.call(reader, \{mapping: values.clientIdProp\}, null, "source") ]})) {\n',
    ...
    buildRecordDataExtractor: function() {
        var me = this,
            ...
            tpl = me.recordDataExtractorTemplate,
            templateData = {
                clientIdProp: modelProto.clientIdProperty,
                fields: modelProto.fields.items,
                exprFn: me.createFieldAccessExpression,
                reader: me
            },
            fn;
        ...
        fn = Ext.functionFactory('fields', tpl.apply(templateData));
        
        return fn.call(me, modelProto.fields);
    },

    destroyReader: function() {
        var me = this;
        
        me.proxy = me.model = me.convertRecordData = me.getId = null;
        me.getTotal = me.getSuccess = me.getMessage = null;
        me.recordDataExtractorTemplate = null;
    }
}, function() {
