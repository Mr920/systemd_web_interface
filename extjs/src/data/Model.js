...
Ext.define('Ext.data.Model', {
    ...
    inheritableStatics: {
         ...
         * **NOTE:** The model returned should have an ID matching the param in the load
         * request.
         * @param {Object} [options] {@link Ext.data.Operation} config object containing
         ...
         * @param {Ext.data.Operation} options.success.operation The operation.
         *
         ...
         * @param {Ext.data.Operation} options.failure.operation The operation.
         *
         ...
         * @param {Ext.data.Operation} options.callback.operation The operation.
        ...
        load: function(id, config) {
            ...
            callback = function(operation) {
                ...
                if (success) {
                    ...
                    // If the server doesn't return a record, then it's a failure, we're trying
                    // to load a model!
                    if (record) {
     ...
     * An Array of `Ext.data.Field` config objects, simply the field 
     * {@link Ext.data.Field#name name}, or a mix of config objects and strings.  If 
     * just a name is given, the field type defaults to `AUTO`.
     * 
     *     // two fields are set:
     *     // - an 'AUTO' field with a name of 'firstName'
     *     // - and an Ext.data.Types#INTEGER field with a name of 'age'
     *     fields: ['firstName', {
     ...
     *
     *  - {@link Ext.data.Store#event-datachanged datachanged}
    ...
    /**
     * @localdoc Destroys the model using the configured proxy.  The destroy action
     * is asynchronous.  Any processing of the destroyed record should be done in a
     * callback.
     *
     *     Ext.define('MyApp..model.User', {
     *         extend: 'Ext.data.Model',
     ...
     *         success: function(record, operation) {
     *             //do something if the load succeeded
     *             // destroy the created record
     *             record.destroy({
     *                 failure: function(record, operation) {
     *                     // do something if the destroy failed
     *                 },
     *                 success: function(record, operation) {
     *                     // do something if the destroy succeeded
     *                 },
     *                 callback: function(record, operation, success) {
     *                     // do something if the destroy succeeded or failed
     *                 }
     *             });
     *         }
     *     });
     *
     * **NOTE:** If a {@link #phantom} record is destroyed it will not be processed via
     * the proxy.  However, any passed `success` or `callback` functions will be called.
     *
     * @inheritdoc #save
     */
            ...
            if (operation.wasSuccessful()) {
                for (storeCount = stores.length; i < storeCount; i++) {
                    ...
                    if (!store.isTreeStore) {
                        ...
                        store.onNodeRemove(me.parentNode, me, false);
                    }
