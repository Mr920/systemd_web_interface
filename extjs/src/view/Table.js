...
Ext.define('Ext.view.Table', {
    ...
    */
    ...
    renderColumnSizer: function(out) {
        ...
        out.push('<colgroup id="', this.id, '-col-sizer" role="presentation">');
    ...
    replaceColumnSizer: function() {
        var me = this,
            newSizer = [],
            sizer = me.el.down('#' + me.id + '-col-sizer'),
            body = me.body;

        me.renderColumnSizer(newSizer);
        newSizer = newSizer.join('');
        ...
        if (body) {
            body.dom.style.width = me.headerCt.getTableWidth() + 'px';
            Ext.core.DomHelper.insertFirst(body, newSizer);
        }
    },
     ...
     */
    focusRow: function(row, delay, /* private */ ifNotFocused) {
        var me = this,
            record,
            focusTask = me.getFocusTask();

        if (delay) {
            focusTask.delay(Ext.isNumber(delay) ? delay : 10, me.focusRow, me, [row, false, ifNotFocused]);
            return;
        }

        if (ifNotFocused && this.getEl().contains(Ext.Element.getActiveElement())) {
            return;
        }
