...
Ext.define('Ext.view.AbstractView', {
    ...
    beforeLayout: function() {
        ...
        if (me.refreshNeeded && !me.pendingRefresh) {
            ...
            // If it is the first refresh, then we must honour the deferInitialRefresh setting which is done in doFirstRefresh
            else {
    ...
    refresh: function() {
        ...
        if (!me.hasListeners.beforerefresh || me.fireEvent('beforerefresh', me) !== false) {
            ...
            // Re-insert the DOM so that the collectNodes works
            if (!me.preserveScrollOnRefresh) {
            ...
            if (records.length < 1) {
                ...
                if (!me.store.loading && (!me.deferEmptyText || hasFirstRefresh)) {
        ...
        if (me.all.getCount()) {
            if (me.dataSource.getCount() === 0) {
                ...
                for (i = indices.length - 1; i >= 0; --i) {
                    ...
                    // Remove the item if it is rendered (It might be out of view range if buffer rendered)
    ...
    onBindStore: function(store, initial, propName) {
        ...
        // if data source is a feature store we skip this part because the feature store
        // does not get bound/unbound, rather it's underlying store property does
        // see Ext.view.View#bindStore() for more details
        if (!initial && propName === 'store' && (!dataSource || !dataSource.isFeatureStore)) {
