...
Ext.define('Ext.grid.header.Container', {
    ...
    applyColumnsState: function(columns) {
        ...
        for (c = 0; c < length; c++) {
            ...
            for (index = count; index--; ) {
                ...
                if (col.getStateId && col.getStateId() == columnState.id) {
                    ...
                    if (i !== index) {
                        me.items.insert(i, me.items.getAt(index));
                        moved = true;
                    }
        ...
        // Because we did not call the container method to move the column, the cached columns needs to be purged from the root headerCt.
        if (moved) {
            me.purgeHeaderCtCache(me);
        }
    },
    ...
    onRemove: function(c) {
        ...
        if (!me.destroying) {
            ...
            // See Ext.layout.container.Container#checkNestedHeader.
            if (me.isGroupHeader && !me.isStackedTargetHeaderParent && ownerCt && !me.items.getCount()) {
    ...
    getVisibleGridColumns: function() {
        ...
        var allColumns = this.getGridColumns(),
            ...
            rootHeader = this.getOwnerHeaderCt(),
            i, column;
     ...
     *
     * See also {@link #getVisibleGridColumns}
