...
Ext.define('Ext.grid.plugin.BufferedRenderer', {
    ...
    onReplace: function(store, startIndex, oldRecords, newRecords) {
        ...
        if (!me.lockingPartner && renderedSize && lastAffectedIndex < rows.startIndex) {

            // Move the index-based NodeCache up or down depending on whether it's a net additing or removal above.
            ...
            if (rows.startIndex === oldStartIndex) {
                ...
                if (rows.startIndex) {
                    ...
                    me.position = me.scrollTop = view.el.dom.scrollTop;
                }
            }
        ...
        if (renderedSize && startIndex > rows.endIndex) {
            ...
            if (recordIncrement > 0) {
                me.onRangeFetched(null, rows.startIndex, Math.min(store.getCount(), rows.startIndex + me.viewSize) - 1, true);
            }
            view.refreshSize(rows.getCount() !== renderedSize);
    ...
    refreshView: function(startIndex) {
        ...
        view.replaceColumnSizer();

        // Only try rendering records if there are any to render.
        if (endIndex - startIndex + 1) {
            me.refreshing = view.refreshing = true;
            me.renderRange(startIndex, endIndex, true);
            view.fireEvent('refresh', view, store.getRange(startIndex, endIndex));
    ...
    onRangeFetched: function(range, start, end, fromLockingPartner) {
        ...
        // The Locking View will take care of adding and removing data for any locking partners (see
        // Ext.grid.locking.view#relayFn), but we need to ensure that we are still at exactly the same
        // scroll position.
        if (lockingPartner && !lockingPartner.disabled) {
            // Sync the locking side and set `fromLockingPartner` to prevent infinite recursion.
            lockingPartner.onRangeFetched(range, start, end, /*fromLockingPartner*/ true);
