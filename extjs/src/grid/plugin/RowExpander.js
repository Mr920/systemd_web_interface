...
Ext.define('Ext.grid.plugin.RowExpander', {
    ...
    setCmp: function(grid) {
        var me = this,
            features = [],
            featuresCfg, rowBodyTpl, lockedTpl;
        ...
        lockedTpl = me.lockedTpl = Ext.XTemplate.getTpl(me, 'lockedTpl');
        ...
        if (lockedTpl) {
            features.push(Ext.apply({
                ...
                getRowBodyContents: function (record) {
                    return lockedTpl.applyTemplate(record.getData());
                }
            }, featuresCfg));
        }
    ...
    addExpander: function() {
        ...
        if (expanderGrid.ownerLockable) {
            me.grid = expanderGrid = expanderGrid.ownerLockable.lockedGrid;
