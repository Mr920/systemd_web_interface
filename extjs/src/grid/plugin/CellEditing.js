...
Ext.define('Ext.grid.plugin.CellEditing', {
    ...
    init: function (grid) {
        ...
        if (Ext.isIE) {
            // Store syncing causes a problem with IE browsers because of the view refresh that
            // is kicked off in AbstractStore#onProxyWrite. Afterwards, IE will then immediately
            // blur the editor after it has been focused which causes the edit to complete (and
            // focus and blur events are asynchronous in IE). This hack will restore the edit
            // after the refresh and focus and blur events have completed.
            //
            // This bug is apparent after entering text and then tabbing to the next editable cell.
            // See EXTJS-15316.
            grid.view.on('refresh', function () {
                ...
                if (me.activeEditor) {
                    setTimeout(function () {
                        me.startEdit(context.record, context.column);
                    }, 1);
                }
            }, me);
        }
    ...
    onEditComplete: function (ed, value, startValue) {
        ...
        if (activeColumn) {
            ...
            context.view.focusRow(context.rowIdx, 100, true);
