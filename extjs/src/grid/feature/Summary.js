...
Ext.define('Ext.grid.feature.Summary', {
    ...
    renderTFoot: function(values, out) {
        ...
        if (!view.store.loading && me.showSummaryRow) {
            record = me.summaryRecord;

            out.push('<tfoot>');
            me.outputSummaryRecord((record && record.isModel) ? record : me.createSummaryRecord(view), values, out);
            out.push('</tfoot>');
        }
    },
    ...
    createSummaryRecord: function (view) {
        var me = this,
            ...
            summaryRecord = me.summaryRecord || (me.summaryRecord = new store.model(null, view.id + '-summary-record')),
            dataIndex, summaryValue;

        // Set the summary field values
        summaryRecord.beginEdit();

        if (remoteRoot && store.proxy.reader.rawData) {
            ...
            for (i = 0; i < colCount; i++) {
                ...
                summaryValue = me.getSummary(store, column.summaryType, dataIndex, info);
                summaryRecord.set(dataIndex, summaryValue);

                // Capture the columnId:value for the summaryRenderer in the summaryData object.
                me.setSummaryData(summaryRecord, column.id, summaryValue);
            }
        }
