...
Ext.define('Ext.grid.feature.Grouping', {
    ...
    init: function(grid) {
        ...
        me.grid.on({
            ...
            staterestore: me.onStateRestore
        });
    ...
    onGroupChange: function() {
        ...
        if (me.hideGroupedHeader) {
            ...
            // Change visibility state of group field's header
    ...
    onStateRestore: function (grid, state) {
        // Because state is init'd and applied after features are created, we need to listen now for anything interesting.
        var storeState = state.storeState,
            groupers = storeState && storeState.groupers,
            grouper;

        if (groupers) {
            grouper = groupers[0];

            grid.store.group(grouper.property, grouper.direction);
            this.refreshIf();
        }
    }
});
