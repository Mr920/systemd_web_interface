...
Ext.define('Ext.form.field.ComboBox', {
    ...
    // private
    assertValue: function(preventCollapse) {
        var me = this,
    ...
    doQuery: function(queryString, forceAll, rawQuery) {
        ...
        if (me.queryCaching && queryPlan.query === me.lastQuery) {
            // this will make sure the combobox is not expanded
            // when it's value has been cleared
            if (!rawQuery) {
                me.expand();
            }
    ...
    doAutoSelect: function() {
        var me = this,
            picker = me.picker,
            lastSelected, selectionModel, itemNode = 0;
        if (picker && me.autoSelect && me.store.getCount() > 0) {
            selectionModel = picker.getSelectionModel();
            // Highlight the last selected item and scroll it into view
            lastSelected = selectionModel.lastSelected;
            itemNode = picker.getNode(selectionModel.selected.length ? lastSelected : 0);
            if (itemNode) {
    ...
    setValue: function(value, doSelect) {
        ...
        if (me.store.loading || !me.store.data.length) {
            me.value = value;
