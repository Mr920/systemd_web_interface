...
Ext.define('Ext.util.Positionable', {
    ...
    getBox: function(contentBox, local) {
        // In IE quirks mode when there is a perfect storm in the dom (like the resizer example)
        // the first read of xy can be incorrect.  Reading the offsetHeight and offsetWidth
        // corrects this so that the following read gets it correct.  See EXTJS-18552
        if (Ext.isIEQuirks) {
    ...
    calculateConstrainedPosition: function(constrainTo, proposedPosition, local, proposedSize) {
        ...
        vector = (me.constrainHeader ? me.header : me).getConstrainVector(
            constrainTo,
            proposedConstrainPosition,
            proposedSize
        );
